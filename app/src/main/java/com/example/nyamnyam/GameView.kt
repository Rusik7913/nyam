...
class GameView(ctx: Context) : View(ctx) {
    ...
    override fun onTouchEvent(e:MotionEvent):Boolean{
        ...
        when(e.action){
            ...
            MotionEvent.ACTION_MOVE -> {
                if(screen==Screen.GAME && !paused){
                    dragging=true
                    // Перерезание: если пользователь проводит пальцем по верёвке — она исчезает.
                    ropes.removeAll { distancePointToSegment(x,y,it.anchorX,it.anchorY,candy.x,candy.y) < 24f }
                }
                return true
            }
    ...
    private fun makeLevel(n:Int):LevelData{
        ...
        rs.add(Rope(cx,cy,230f))
        if(n>=4) rs.add(Rope(w*.48f,h*.20f,190f))
        if(n>=8) rs.add(Rope(w*.18f,h*.47f,155f))

        val bs=mutableListOf<Bubble>()
        if(n%3==0) bs.add(Bubble(w*.52f,ground-90f,35f))
        if(n>=6) bs.add(Bubble(w*.72f,ground-150f,30f))

        val ss=mutableListOf<Spike>()
        if(n>=3) ss.add(Spike(w*.55f,ground,25f))
        if(n>=7) ss.add(Spike(w*.67f,ground,25f))
        if(n>=12) ss.add(Spike(w*.42f,ground,25f))

        val st=mutableListOf<Star>()
        st.add(Star(w*.45f,h*.30f))
        st.add(Star(w*.60f,h*.43f))
        st.add(Star(w*.78f,h*.52f))

        return LevelData(cx,cy,targetX,targetY,rs,bs,ss,st)
    }
